...
```powershell
git clone https://github.com/TEC-The-ELidoras-Codex/TEC_Q5EP.git
cd TEC_Q5EP
python -m venv .venv
...
The capture UI lives at [http://localhost:5173](http://localhost:5173) and the API at [http://localhost:8000](http://localhost:8000).

> Prefer one-click workflows? See `docs/VSCode_Runbook.md` for ready-to-paste VS Code tasks, or run the PowerShell helper `.\run_me.ps1` to bootstrap and start the API in one go.
...
- `docs/VSCode_Runbook.md` — step-by-step setup inside VS Code, including debugger and automated tasks.
- `docs/COMMANDS_CHEATSHEET.md` — copy-ready PowerShell commands for day-to-day operations.
- `run_me.ps1` — bootstrap script that provisions the venv, installs dependencies, and launches Uvicorn on demand.
...
- `data/runs/<id>/` — raw submissions captured by the `/submit` endpoint.
- `data/evidence_packs/` — zipped bundles generated by `/pack/{id}` for archival.
- `data/agents/media_companion/` — media agent artifacts (audio manifests, analyses, summaries) grouped by run id.
